...
public class client extends node{
		...
		public ArrayList < packet> getPacketsRequested(){
			return (ArrayList<packet>) packets_requested;
		}
...
//Currently hard-coded such that a client can only request from a host server node at the start of the simulation
			...
			for(int i=0; i<=packet_count; i++) {
				node source = connected_devices.get(rand.nextInt(0,connected_devices.size()-1));
...
//				}
				...
				Integer size = rand.nextInt(64,bound);
